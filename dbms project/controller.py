...
load_dotenv('db.env')  # Imports environemnt variables from the '.env' file
...
def checkUser(username, password=None):
    cmd = f"Select count(username) from login where username='{username}' and password='{password}'"
    cursor.execute(cmd)
    cmd = None
